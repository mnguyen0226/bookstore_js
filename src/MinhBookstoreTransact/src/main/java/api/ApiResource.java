...
public class ApiResource {
    ...
    public Category categoryByName(
            @PathParam("category-name") String categoryName, @Context HttpServletRequest httpRequest) {
        try {
    ...
    public List<Book> booksByCategoryName(@PathParam("category-name") String categoryName,
            @Context HttpServletRequest httpRequest) {
        try {
            Category category = categoryDao.findByName(categoryName); // find the category by name
            if (category == null) {
                throw new ApiException(String.format("No such category name: %d", categoryName));
            }
            return bookDao.findByCategoryId(category.getCategoryId());
